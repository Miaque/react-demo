...
const handleStream = (response: Response, onData: IOnData, onCompleted?: IOnCompleted) => {
...
export const ssePost = (url: string, fetchOptions: FetchOptionType, otherOptions: IOtherOptions) => {
  const { isPublicAPI = false, onData, onCompleted, onError, getAbortController } = otherOptions
  const abortController = new AbortController()
